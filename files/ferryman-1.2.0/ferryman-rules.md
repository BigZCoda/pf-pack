...
**Shipped:** 2026-07-20 rebuild — settled pipeline (push live via push.py, mirror pull, verify). **Updated 2026-08-28 (ferryman 1.2.0):** PATCH correction path, participants on import, folder move/remove, task delete, create routes for projects/folders/contacts, API-catalog snapshot.
...
| — participants | ✅ | ✅ (on import) | — | `participants[]` on `POST /transcripts/import` | first-class: push-supplied participants are NEVER overwritten — the brain resolves names locally, the app never guesses. **push.py sends none yet (known open gap).** | LIVE (endpoint) / GAP (push.py) |
...
**Last touched:** 2026-08-28 (ferryman 1.2.0): capability matrix refreshed from the app's self-describing catalog — PATCH metadata correction, participants on import (push.py gap open), folder move/remove, task delete, create routes for projects/folders/contacts, Topics bullet-format rule, API-catalog snapshot row. Previous: 2026-07-20 rebuild, genericized from the reference implementation.
