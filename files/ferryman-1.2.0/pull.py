...
def _cache(path, raw):
    ...
    # path "/" (the API root catalog) slugs to "" -> ".json", a dotfile that is easy to
    # miss and awkward to read. Name the root explicitly.
    stem = re.sub(r"[^a-zA-Z0-9]+", "_", path.strip("/")) or "root"
    (CACHE / f"{stem}.json").write_bytes(raw)
...
def refresh_endpoint_catalog():
    """Overwrite pf-app-api-catalog.json from GET /api/v1 (the app's self-describing catalog).

    This is the app's own answer to "what can you do / which token can call what". It is the
    ONLY capability source that cannot drift from the app, because the app writes it.

    Stamped with _fetchedAt because a cached catalog is a snapshot, not a fact: a stale copy
    once hid seven newly shipped routes for 35 days, so an agent trusting it would have
    concluded the app cannot create a contact or a project. The stamp is what makes staleness
    visible to the next reader.
    """
    # Absolute URL on purpose: _get() would build "{BASE}/" and a trailing slash is an
    # unmatched /api/* path, which this app answers with HTTP 200 + the website's HTML.
    # Assert the shape so that defect can never write garbage into the catalog under a 200.
    data, raw = _get(BASE)
    if not isinstance(data, dict) or "callableByYou" not in data:
        raise ValueError("API root did not return the catalog shape (SPA-HTML-under-200?)")
    _cache("", raw)
    data["_fetchedAt"] = datetime.now(timezone.utc).isoformat(timespec="seconds")
    data["_source"] = f"GET {BASE} (machine-written by skills/ferryman/pull.py — never hand-edit)"
    data["_staleWarning"] = (
        "A snapshot, not the live surface. If this is more than ~7 days old and you have a token "
        "and network, re-run `python skills/ferryman/pull.py --pull` or GET the API root directly. "
        "callableByYou reflects the token that fetched it (personal pf_tok_, not admin pfk_)."
    )
    (HOLON / "pf-app-api-catalog.json").write_text(
        json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")
    return len(data.get("callableByYou") or [])
...
def refresh_snapshots():
    ...
    try:
        n = refresh_endpoint_catalog()
        done.append(f"pf-app-api-catalog.json({n} callable)")
    except urllib.error.HTTPError as e:
        done.append(f"pf-app-api-catalog.json:HTTP{e.code}")
    except Exception as e:
        done.append(f"pf-app-api-catalog.json:{type(e).__name__}")
    simple = {
