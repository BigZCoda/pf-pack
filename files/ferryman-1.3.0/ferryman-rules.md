...
**Shipped:** 2026-07-20 rebuild — settled pipeline (push live via push.py, mirror pull, verify). **Updated 2026-08-28 (ferryman 1.2.0):** PATCH correction path, participants on import, folder move/remove, task delete, create routes for projects/folders/contacts, API-catalog snapshot. **Updated 2026-09-03 (ferryman 1.3.0):** push.py's `source` key removed (the schema rejects it 422; every 1.2.0 push failed at validate), participants shipped via `--participants`, `--base` flag.

## Current truth (read this first; 1.3.0)

- **Never send a `source` key** in a transcript push. The import schema rejects it `422 Unrecognized field(s): source`; the server sets it. push.py 1.2.0 sent it, so every push failed at validate with no visible error.
- **Participants ride in the frontmatter as a YAML list** (`push.py --participants "A,B"`). A top-level `participants` JSON key is rejected the same way.
- **Resolve participants, never mentions.** Participants are the closed set of people in the room; mentions are people talked about and get no id. Speaker strings are speech-recognition output — resolve them to the exact contact name before passing them.
- **Read the import response's `contacts` block** (`matched / created / ambiguous / skipped / enrichmentQueued`). `ambiguous` = matched two or more contacts, linked to none, dropped silently.
- **Print the response keys before reporting any negative.** A field that is not on that endpoint returns nothing and looks like real emptiness (the reference brain hit this three times).
- Capability questions: live `GET /api/v1` → the `_fetchedAt`-stamped catalog (say its date) → this file's matrix, in that order.
...
**Capability questions get answered from the app, not from prose.** `GET /api/v1` is self-describing (it returns `callableByYou` for your token); every pull also snapshots it to `pf-app-holon/pf-app-api-catalog.json`, stamped `_fetchedAt`. Trust the live call first, the stamped snapshot second (say its date out loud when you cite it), this file's matrix third. A snapshot is not a fact — check the stamp.
...
4. **PULL (app → brain):** run `python skills/ferryman/pull.py --pull`. Mirrors new/changed transcripts + snapshots + the API catalog. Idempotent (UUID + `updatedAt`).
...
The canonical list of what your brain may push/edit vs. what is read-only. Your personal token (`pf_tok_*`) only. *(Refreshed 2026-08-28 from the app's self-describing catalog; on any doubt, `GET /api/v1` wins.)*
...
| — metadata correction | | | ✅ | `PATCH /transcripts/:id` | change date / conversationType / tags WITHOUT resending the body — **THE correction path**; use instead of a re-push | LIVE |
| — participants | ✅ | ✅ (on import) | — | `participants[]` on `POST /transcripts/import` | first-class: push-supplied participants are NEVER overwritten — the brain resolves names locally, the app never guesses. **push.py 1.3.0 sends them via `--participants` as a frontmatter YAML list** (a top-level key is rejected 422). Read the response's `contacts` block: `ambiguous` = linked to nobody. | LIVE |
| — folder membership | ✅ read | ✅ (on import) | ✅ move / remove | `POST /transcripts/import` · `POST /transcripts/:id/folders/move` · `DELETE /transcripts/:id/folders/:folderId` | assign on import, multi-folder OK; remove = membership only, never deletes the transcript | LIVE |
| — id-less dedup | | ✅ | | `POST /transcripts/import` | id-less creates matched ±2-day + participant/title → `possibleDuplicate:true` + scored `duplicateCandidates[]`; NEVER auto-merges; resolve in the app's review view. Upserts-by-id are NOT dup-checked. | LIVE |
| **Task** | ✅ | ✅ | ✅ (status/date) · ✅ delete | `POST /tasks`, `PATCH /tasks/:id`, `DELETE /tasks/:id` | delete = your own action items only | LIVE |
...
| **Folders / projects** | ✅ | ✅ | ❌ (edit is admin-only) | `POST /projects` · `POST /projects/:id/folders` | create a project `{ title, description? }` / a folder under a project you own; you still can't EDIT one | CREATE LIVE |
| **Contacts** | ✅ | ✅ | ❌ | `POST /contacts` | exact-name match returns the EXISTING contact — no duplicate. ⚠️ matching is NOT nickname-aware: a nickname or name variant creates a SECOND contact — resolve to the exact existing contact name before creating | CREATE LIVE |
...
4. **Read-only means read-only.** Do not attempt to push context documents / presets — the write path does not exist. (Folders, projects, and contacts DO have create routes now — see the matrix — but no personal-token edit.)
5. **Validate before commit** (`/import/validate`) on every transcript push.
6. **Re-push a correction only WITH the record's `id`** — a push with no `id` creates, and your personal token cannot delete the resulting duplicate. **Metadata mistakes don't need a re-push at all:** `PATCH /transcripts/:id` fixes date/type/tags in place.
...
- **Your personal token cannot DELETE a transcript** (there is no personal-token delete route) — a duplicate is permanent from your side. **It CAN `PATCH /transcripts/:id`** (date / conversationType / tags — the correction path) and move/remove folder membership. So: validate first, push once, upsert-with-id for content corrections, PATCH for metadata corrections.
- A `## Topics` section populates the record's **`tags`** field — tags = topics; that's the intended home. ⚠️ **Topics must be `- ` bullet lines, one per line** — a comma-separated Topics paragraph validates, pushes 201, and lands ZERO tags silently (fixable post-hoc via `PATCH /transcripts/:id {"tags":[...]}`).
...
- **Scope tier:** personal (`pf_tok_*`). Discovery (`GET /api/v1`) returns the `callableByYou` list for this token — trust it over any cached inventory, including this file.

## 3. Endpoint inventory (endpoint → file it feeds)

| Endpoint | Output file(s) | Pull behavior |
|---|---|---|
| `GET /api/v1` (discovery) | `pf-app-api-catalog.json` + `.cache/root.json` | The app's self-describing catalog, `_fetchedAt`-stamped — refreshed on every pull; THE capability source |
...
| `GET /projects/:id/folders` | (your folder table in the runbook) | Discover your folder UUIDs — refetch at push time; the runbook table is a cache |

**Raw responses** for every call land in `.cache/` (filename = endpoint path, `/` → `_`; the API root caches as `root.json`). `.cache/` never syncs anywhere.
...
App always wins inside this folder. Mirror files are never edited in place — local edits are silently lost on the next pull and the app never learns of them. If app data is wrong, fix it in the app (or via the PATCH correction path) and re-pull. (Brain wins on brain-side synthesis — but that lives outside this folder by definition.)
...
**Last touched:** 2026-09-03 (ferryman 1.3.0): `source` key removed from push.py (was failing validate on every push), participants LIVE via `--participants`, Current-truth block added at the top, `--base` flag. Previous: 2026-08-28 (ferryman 1.2.0): capability matrix refreshed from the app's self-describing catalog — PATCH metadata correction, participants on import (push.py gap open), folder move/remove, task delete, create routes for projects/folders/contacts, Topics bullet-format rule, API-catalog snapshot row. Previous: 2026-07-20 rebuild, genericized from the reference implementation.
