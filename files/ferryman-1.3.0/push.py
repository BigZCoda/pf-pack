...
Usage:
  ...
  python push.py --push-summary <intake-report.md> --raw <raw.txt>
                 ...
                 [--date YYYY-MM-DD] [--title "<title>"] [--participants "Name,Name"]
                 [--base <url>] [--dry-run]
...
Safety:
  ...
  - No "source" key is sent. The import schema rejects it 422 ("Unrecognized field(s): source");
    the server sets source itself.
  - participants ride in the FRONTMATTER as a YAML list (--participants), never as a top-level
    key -- a top-level participants key is rejected the same way. They are what links people to
    the record, so a push without them lands with nobody attached.
...
1.3.0 (2026-09-03):
  - "source" key REMOVED from both payload builders (build() and build_summary_payload()).
    The import schema rejects it 422 "Unrecognized field(s): source" and the server sets
    source itself, so every push that carried the key was failing at validate.
  - --participants "Full Name,Full Name" writes a participants: YAML list into the markdown
    FRONTMATTER (the only accepted path -- a top-level JSON key is rejected the same way),
    quoted via json.dumps so names with punctuation stay valid YAML. Shown in --dry-run output.
  - --base <url> retargets the API base (e.g. a local dev server). A dev server may share the
    PRODUCTION database -- a push there is a real write to real data, not a sandbox.
"""
...
def build(path):
    ...
    if "interview" in low or "onboarding" in low:
        ...
        ctype = "Other"
    # No "source" key: the import schema rejects it 422 ("Unrecognized field(s): source");
    # the server sets source itself.
...
def build_summary_payload(args):
    ...
    # participants MUST ride in the frontmatter as a YAML list: a top-level "participants"
    # payload key is rejected 422 "Unrecognized field(s)". json.dumps gives YAML-safe quoting.
    parts = [p.strip() for p in (args.participants or "").split(",") if p.strip()]
    if parts:
        fm.append("participants: [" + ", ".join(json.dumps(p) for p in parts) + "]")
    fm.append("---")
    md = "\n".join(fm) + "\n"
    ...
    # No "source" key. The import schema rejects it 422 ("Unrecognized field(s): source"),
    # so a payload carrying it fails at validate. The server sets source itself.
    payload = {"markdown": md}
    if args.folder:
...
def push_summary(args):
    ...
    if args.dry_run:
        ...
        print(f"  participants: {', '.join(meta['participants']) if meta['participants'] else '(NONE -- nobody will be linked to this record)'}")
...
def _apply_base(args):
    """Let --base retarget the module-level BASE (e.g. a local dev server; see --base help)."""
    global BASE
    if getattr(args, "base", None):
        BASE = args.base.rstrip("/")
        print(f"BASE OVERRIDE: {BASE}")
...
def main():
    ...
    ap.add_argument("--base", metavar="URL", help=(
        f"override the API base URL (default {BASE}), e.g. http://localhost:5000/api/v1 to "
        "exercise a local dev build before it ships. NOTE: a dev server may share the PRODUCTION "
        "database -- a push there is a real write to real data, not a sandbox."))
    ap.add_argument("--participants", metavar="NAMES", help=(
        "comma-separated participant names, e.g. 'Full Name,Full Name'. Goes into the frontmatter "
        "as a YAML list -- the ONLY accepted path (a top-level participants key is rejected 422 as "
        "an unrecognized field). Send canonical full names, NOT raw transcriber labels: the app "
        "matches these against your contacts to link people to the conversation. Omit rather than "
        "guess -- an unmatched name creates a new contact row."))
